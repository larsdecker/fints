...
import { PinTanClient, StandingOrderUpdate, TanRequiredError } from "fints";
import { BaseConfig } from "../config";
import { setLevel } from "../logger";
import { promptTan } from "../tan";

export class UpdateStandingOrderOptions extends BaseConfig {
    ...
    @option({ required: true, flag: "s", description: "Next execution date (YYYY-MM-DD)." })
    public start: string;
...
@command({ description: "Update an existing standing order." })
export default class extends Command {
    @metadata
    public async execute(options: UpdateStandingOrderOptions) {
        const { verbose, serializer, iban, orderId, amount, currency, purpose, creditorName, creditorIban, creditorBic, debitorName, debitorIban, debitorBic, start, timeUnit, interval, executionDay, end, ...config } = options;
        setLevel(verbose);
        ...
        const update: StandingOrderUpdate = {
            orderId,
            payment: {
                amount,
                currency,
        ...
        try {
            const result = await client.updateStandingOrder(account, update);
            ...
            if (error instanceof TanRequiredError) {
                ...
                const result = await client.completeUpdateStandingOrder(error.dialog, error.transactionReference, tan);
                console.info(serializer(result));
            } else {
                throw error;
            }
